<?php
	...
	function getbookprice($isbn){
		$conn = db_connect();
		$query = "SELECT book_price, book_isbn FROM books WHERE book_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getPubName($conn, $pubid){
		$query = "SELECT publisher_name, publisher_address FROM publisher WHERE publisherid = '$pubid'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getOrder($conn){
		$query = "SELECT orders.orderid, orders.amount, orders.date, orders.ship_name, orders.ship_address, orders.ship_zip_code, order_items.orderid, order_items.book_isbn, order_items.quantity, books.book_isbn, books.book_title, order_items.book_isbn from orders, order_items, books WHERE order_items.orderid = orders.orderid AND books.book_isbn = order_items.book_isbn ";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getPublisher($conn){
		$query = "SELECT publisher_name, publisher_address,publisherid from publisher ORDER BY publisherid DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function Customer($conn){
		$query = "SELECT customerid, name, address, city, zip_code, country from customers ORDER BY customerid DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;		
